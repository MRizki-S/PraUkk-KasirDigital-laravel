...
@section('title', 'Kamar')
@section('content')
    <h3 class="text-2xl text-gray-700 font-semibold">Kamar</h3>
    <p class=" text-gray-500 border-b">Home / kamar </p>
    {{-- {{ kamar }} --}}
            ...
            <tbody>
                @foreach ($kamar as $item)
                    <tr
                        ...
                        <td class="px-6 py-4">
                            Rp. {{ $item->kamarType->harga_permalam }}
                        </td>
                        <td class="px-6 py-4 flex gap-2 flex-wrap">
                            ...
                            <a href="#" type="button" data-modal-target="deleteKamarModal-{{ $item->id }}"
                                data-modal-show="deleteKamarModal-{{ $item->id }}"
...
<div id="tambahKamarModal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <form class="relative bg-white rounded-lg shadow dark:bg-gray-700" action="/tambahKamar" method="POST">
            @csrf
            ...
            <div class="p-6 space-y-6">
                {{-- tIPE KAMAR --}}
                <div class="mb-4">
                    <label for="kamar_type_id" class="block text-gray-700 font-medium mb-2">Tipe Kamar</label>
                ...
                {{-- no kamar --}}
                <div class="mb-4">
                    <label for="no_kamar" class="block text-gray-700 font-medium mb-2">No Kamar</label>
                    <input type="text" id="no_kamar" name="no_kamar" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none focus:border-blue-400 placeholder-gray-400"
                        placeholder="No Kamar" value={{ old('no_kamar') }}>
                    @error('no_kamar')
...
@foreach ($kamar as $item)
    <div id="editKamarModal-{{ $item->id }}" tabindex="-1" aria-hidden="true"
        class="fixed top-0 left-0 right-0 z-50 items-center justify-center hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-2xl max-h-full">
            <!-- Modal content -->
            <form class="relative bg-white rounded-lg shadow dark:bg-gray-700" action="/editKamar/{{ $item->id }}"
                method="POST">
                @method('put')
                @csrf
                ...
                <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-400">
                    ...
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="editKamarModal-{{ $item->id }}">
                        <i class="fa-solid fa-x"></i>
                        <span class="sr-only">Close modal</span>
                    </button>
                </div>
                ...
                <div class="p-6 space-y-6">
                     ...
                     <!-- Pilihan kamar type -->
                     <div>
                        <label for="kamar_type_id" class="block text-gray-700">Tipe Kamar</label>
                        <select name="kamar_type_id" id="kamar_type_id"
                            class="w-full px-4 py-2 border rounded-lg">
                            @foreach ($kamarType as $dataKamarType)
                                <option value="{{ $dataKamarType->id }}"
                                    {{ $item->kamar_type_id == $dataKamarType->id ? 'selected' : '' }}>
                                    {{ $dataKamarType->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="no_kamar" name="no_kamar" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none focus:border-blue-400 placeholder-gray-400"
                            placeholder="No Kamar" value={{ $item->no_kamar }}>
                        @error('no_kamar')
                            <p class=" text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div
                    ...
                    <button type="button"
                        ...
                        data-modal-hide="editKamarModal-{{ $item->id }}">Cancel</button>
                    <button type="submit"
...
@foreach ($kamar as $item)
    <div id="deleteKamarModal-{{ $item->id }}" tabindex="-1" aria-hidden="true"
        class="fixed top-0 left-0 right-0 z-50 items-center justify-center hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-2xl max-h-full">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-400">
                    ...
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="deleteKamarModal-{{ $item->id }}">
                        <i class="fa-solid fa-x"></i>
                        <span class="sr-only">Close modal</span>
                    </button>
                </div>
                ...
                <div class="p-6 space-y-6">
                    <div class="flex items-center justify-center space-x-4">
                        ...
                        <p class="text-gray-700 dark:text-white">Apakah Anda yakin ingin menghapus kamar ini?</p>
                    </div>
                </div>
                <!-- Modal footer -->
                <div
                    class="flex items-center justify-end p-6 space-x-3 border-t border-gray-200 rounded-b dark:border-gray-600">
                    <a href="/deleteKamar/{{ $item->id }}"
                    ...
                    <button type="button"
                        class="text-white bg-yellow-500 hover:bg-yellow-600 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-yellow-500 dark:hover:bg-yellow-600 dark:focus:ring-yellow-800"
                        data-modal-hide="deleteKamarModal-{{ $item->id }}">Cancel</button>
                </div>
            </div>
        </div>
    </div>
